...
def send_repair_status_email_task(repair_id: str) -> None:
    ...
    if not repair or not repair.customer or not repair.customer.email:
        return
    public_url = f"/api/v1/repairs/track/{repair.public_lookup_token}" if repair.public_lookup_token else ""
    context = {"customer": repair.customer, "repair": repair, "public_url": public_url}
    EmailSender().send_template([repair.customer.email], "Repair status updated", "notifications/repair_status_changed.html", context)
